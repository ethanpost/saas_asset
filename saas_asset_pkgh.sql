...
create or replace package saas_asset_pkg as 
   ...
   function get_asset_id (
      ...
      p_root_id in number) return number;
   ...
   function does_type_exist (
      p_name in varchar2,
      p_root_id in number) return boolean;

   function get_asset_type (
      p_name in varchar2,
      p_root_id in number) return saas_asset_types%rowtype;

   function get_asset_type_name_by_id (
      p_id number,
      p_root_id in number) return varchar2;
   ...
   function create_asset (
      p_name in varchar2,
      p_type_name in varchar2,
      p_root_id in number) return saas_asset%rowtype;
